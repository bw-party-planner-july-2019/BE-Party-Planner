...
describe('Party Model', () => {
	...
	describe('add', () => {
		it('add party into database', async () => {
			const newParty = await Party.add({ user_id: 1, party_name: 'Don promotion' });
			expect(newParty.party_name).toBe('Don promotion');
		});
		it('adding 2 parties to the database', async () => {
			const newParty1 = await Party.add({ user_id: 1, party_name: 'Don promotion' });
			const newParty2 = await Party.add({ user_id: 1, party_name: 'Hawaiian' });
	...
	describe('remove', () => {
		it('removes party correctly', async () => {
			const partyOne = await Party.add({ user_id: 1, party_name: 'Don promotion' });
		...
		it('removes party correctly when adding more than one', async () => {
			const partyOne = await Party.add({ user_id: 1, party_name: 'Don promotion' });
			const partyTwo = await Party.add({ user_id: 1, party_name: 'Hawaiian' });
			const partyThree = await Party.add({ user_id: 1, party_name: 'party time' });
			const removeparty = await Party.remove(1);
